...
$(document).ready(function (){
  ...
  function set_style(target, value)
  {
    // XXX: disable sticky mode if sidebar (or at least toclist) is taller than window?
    if(value <= 0){
      target.css({marginLeft: "", position: "", top: "", left: "", bottom: ""});
    }
    else if (value < holder.height() - target.height()){
      target.css({marginLeft: 0, position: "fixed", top: 0, left: holder.offset().left, bottom: ""});
    }
    else{
  ...
  function update_sticky(){
    ...
    set_style(sidebar, offset);
    set_style(toggle, offset);
  };
